...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, refer_bbox, value, value_shapes, value_mask=None):
        """
        Perform forward pass for multiscale deformable attention with OBB support.

        Args:
            query (torch.Tensor): Tensor with shape [bs, query_length, C].
            refer_bbox (torch.Tensor): Tensor with shape [bs, query_length, n_levels, 2/4/5],
                range in [0, 1] for coordinates, including padding area.
                - 2D: (x, y) coordinates
                - 4D: (x, y, w, h) axis-aligned bbox
                - 5D: (x, y, w, h, r) oriented bbox with rotation
            value (torch.Tensor): Tensor with shape [bs, value_length, C].
            value_shapes (list): List with shape [n_levels, 2], [(H_0, W_0), (H_1, W_1), ..., (H_{L-1}, W_{L-1})].
            value_mask (torch.Tensor, optional): Tensor with shape [bs, value_length].
        ...
        # Calculate sampling locations based on reference bbox dimensions
        ...
        if num_points == 2:
            ...
            # Oriented bounding box (x, y, w, h, r)
            # Extract components
            center_xy = refer_bbox[:, :, None, :, None, :2]  # (bs, len_q, 1, n_levels, 1, 2)
            wh = refer_bbox[:, :, None, :, None, 2:4]  # (bs, len_q, 1, n_levels, 1, 2)
            rotation = refer_bbox[:, :, None, :, None, 4:5]  # (bs, len_q, 1, n_levels, 1, 1)

            # Scale sampling offsets by bbox size
            scaled_offsets = sampling_offsets / self.n_points * wh * 0.5  # (bs, len_q, n_heads, n_levels, n_points, 2)

            # Apply rotation transformation
            cos_r = torch.cos(rotation)  # (bs, len_q, 1, n_levels, 1, 1)
            sin_r = torch.sin(rotation)  # (bs, len_q, 1, n_levels, 1, 1)

            # Create rotation matrix and apply to offsets
            # For each offset (dx, dy), rotated offset is:
            # dx_rot = dx * cos(r) - dy * sin(r)
            # dy_rot = dx * sin(r) + dy * cos(r)
            dx = scaled_offsets[:, :, :, :, :, 0:1]  # (bs, len_q, n_heads, n_levels, n_points, 1)
            dy = scaled_offsets[:, :, :, :, :, 1:2]  # (bs, len_q, n_heads, n_levels, n_points, 1)

            dx_rot = dx * cos_r - dy * sin_r
            dy_rot = dx * sin_r + dy * cos_r

            rotated_offsets = torch.cat([dx_rot, dy_rot], dim=-1)  # (bs, len_q, n_heads, n_levels, n_points, 2)

            # Add to center coordinates
            sampling_locations = center_xy + rotated_offsets

        else:
            raise ValueError(f"Last dim of reference_points must be 2, 4, or 5, but got {num_points}.")

        # Ensure sampling locations are within valid range [0, 1]
        sampling_locations = torch.clamp(sampling_locations, 0.0, 1.0)
...
class DeformableTransformerDecoderLayerOBB(nn.Module):
    def __init__(self, d_model=256, n_heads=8, d_ffn=1024, dropout=0.0, act=nn.ReLU(), n_levels=4, n_points=4):
        ...
        # 防止梯度爆炸
        self.angle_norm = nn.LayerNorm(d_model)
        self.angle_dropout = nn.Dropout(0.1)

        # 新增：角度编码线性层（可选）
        self.angle_proj = nn.Linear(2, d_model)  # 输入 sinθ, cosθ，输出与 query 同维度

        # 初始化 angle_proj 权重为较小值
        nn.init.xavier_uniform_(self.angle_proj.weight, gain=0.1)
    ...
    def encode_angle(self, angle):
        """
        将角度信息编码为 sin 和 cos，并映射到 d_model 维度。
        angle: Tensor(bs, num_queries, 1)
        返回: Tensor(bs, num_queries, d_model)
        """
        sin_cos = torch.cat([angle.sin(), angle.cos()], dim=-1)  # bs, nq, 2
        return self.angle_proj(sin_cos)  # bs, nq, d_model
    ...
    def forward(self, embed, refer_bbox, feats, shapes, padding_mask=None, attn_mask=None, query_pos=None):
        """
        Args:
            embed: Query Embedding (bs, num_queries, d_model)
            refer_bbox: (bs, num_queries, 5) -> xywh + angle
            feats: multi-scale features (bs, total_num_keys, d_model)
            shapes: List of (H, W)

        Important: Angle encoding is injected AFTER self-attention but BEFORE cross-attention.
        This ensures self-attention can properly model query-to-query interactions (duplicate
        suppression) without angle interference, while angle information still guides the
        cross-attention to locate rotated objects.
        """
        bs, nq, _ = embed.shape

        # ============ 1. 自注意力 (without angle encoding for proper duplicate suppression) =============
        q = k = self.with_pos_embed(embed, query_pos)
        tgt = self.self_attn(q, k, embed, attn_mask=attn_mask)[0]
        embed = self.norm1(embed + self.dropout1(tgt))
        if torch.isnan(embed).any():
            print("NaN detected in embed after self-attn")
            raise RuntimeError("NaN in decoder embed")

        # ============ 2. 注入角度编码 (for rotation-aware cross-attention) =============
        angle = refer_bbox[..., 4:5]  # (bs, nq, 1)
        angle_feature = self.encode_angle(angle)  # (bs, nq, d_model)
        angle_feature = self.angle_norm(angle_feature)
        embed = embed + 0.1 * (self.angle_dropout(angle_feature))
        if torch.isnan(embed).any():
            print("NaN detected in embed after angle encoding")
            raise RuntimeError("NaN in decoder embed")

        # ============ 3. 交叉注意力 =============
        ref_bbox = refer_bbox.unsqueeze(2)  # bs, nq, 1, 5
        tgt2 = self.cross_attn(self.with_pos_embed(embed, query_pos), ref_bbox, feats, shapes, padding_mask)
        embed = self.norm2(embed + self.dropout2(tgt2))
        if torch.isnan(embed).any():
            print("NaN detected in embed after cross-attn")
            raise RuntimeError("NaN in decoder embed")
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        # 用 torch.cat 替代切片赋值，避免原地操作导致 autograd 错误
        refer_bbox = torch.cat([
            refer_bbox[..., :4].sigmoid(),
            refer_bbox[..., 4:]
        ], dim=-1)
        for i, layer in enumerate(self.layers):
            ...
            xy = torch.sigmoid(bbox[..., :2] + inverse_sigmoid(refer_bbox[..., :2]))
            wh = torch.sigmoid(bbox[..., 2:4] + inverse_sigmoid(refer_bbox[..., 2:4])).clamp(min=0.01, max=1.0)
            angle = bbox[..., 4:5] + refer_bbox[..., 4:5]
            refined_bbox = torch.cat([xy, wh, angle], dim=-1)
            ...
            if self.training:
                ...
                if i == 0:
                    ...
                    xy2 = torch.sigmoid(bbox[..., :2] + inverse_sigmoid(last_refined_bbox[..., :2]))
                    wh2 = torch.sigmoid(bbox[..., 2:4] + inverse_sigmoid(last_refined_bbox[..., 2:4])).clamp(min=0.01, max=1.0)
                    angle2 = bbox[..., 4:5] + last_refined_bbox[..., 4:5]
                    cp_last_refined_bbox = torch.cat([xy2, wh2, angle2], dim=-1)
                    dec_bboxes.append(cp_last_refined_bbox)
