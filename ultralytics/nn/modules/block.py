...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad, Conv_AdaptiveRotatedConv2d
...
class HGBlock_AdaptiveRotatedConv2d(nn.Module):
    """
    HG_Block of PPHGNetV2 with 2 convolutions and LightConv.
    ...
    def __init__(self, c1, cm, c2, k=3, n=6, lightconv=False, shortcut=False, act=nn.ReLU()):
        ...
        block = LightConv if lightconv else Conv
        self.m = nn.ModuleList(block(c1 if i == 0 else cm, cm, k=k, act=act) for i in range(n))
        self.sc = Conv_AdaptiveRotatedConv2d(c1 + n * cm, c2 // 2,k, 1, act=act)  # squeeze conv
        self.ec = Conv_AdaptiveRotatedConv2d(c2 // 2, c2, k, 1, act=act)  # excitation conv
        self.add = shortcut and c1 == c2
