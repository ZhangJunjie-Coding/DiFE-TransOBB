...
from .transformer import MLP, DeformableTransformerDecoder, DeformableTransformerDecoderLayer, \
    DeformableTransformerDecoderLayerOBB
...
class RTDETRDecoder(nn.Module):
    ...
    def __init__(
        ...
        decoder_layer = DeformableTransformerDecoderLayerOBB(hd, nh, d_ffn, dropout, act, self.nl, ndp)
    ...
    def forward(self, x, batch=None):
        ...
        from ultralytics.models.utils.ops import get_cdn_group_obb
        if any(torch.isnan(t).any() for t in x if isinstance(t, torch.Tensor)):
            print("NaN detected in embed")
            raise RuntimeError("NaN in decoder embed")
        # Input projection and embedding # 获取特征图
        feats, shapes = self._get_encoder_input(x)

        # Prepare denoising training # 对比去噪
        dn_embed, dn_bbox, attn_mask, dn_meta = get_cdn_group_obb(
            batch,
            self.nc,
            self.num_queries,
        ...
        if torch.isnan(embed).any():
            ...
            raise RuntimeError("NaN in decoder embed")
        # TODO ing
        # Decoder 是 DeformableTransformerDecoder
    ...
    def _generate_anchors(self, shapes, grid_size=0.05, dtype=torch.float32, device="cpu", eps=1e-2):
        ...
        # 正确添加旋转角度维度
        rotation = torch.zeros_like(anchors[..., :1])  # (1, h*w*nl, 1) - 与anchors的前两个维度保持一致
        anchors = torch.cat([anchors, rotation], dim=-1)  # (1, h*w*nl, 5)
    ...
    def _get_decoder_input(self, feats, shapes, dn_embed=None, dn_bbox=None):
        ...
        refer_bbox = torch.cat([
            refer_bbox[..., :4],
            refer_bbox[..., 4:5].tanh() * (math.pi / 2)
        ], dim=-1)

        enc_bboxes = torch.cat([
            refer_bbox[..., :4].sigmoid().clamp(min=0.01, max=1.0),
            refer_bbox[..., 4:5]
        ], dim=-1)
