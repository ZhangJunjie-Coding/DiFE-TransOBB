...
from ultralytics.nn.AddModules import SimpleStem
from ultralytics.nn.AddModules import FrequencySpatialFPN
...
def _clear_thop_buffers(mod):
    # 递归清理当前模块及其所有子模块上的 thop 缓存
    for s in mod.modules():
        # 清理注册为 buffer 的条目
        buf = getattr(s, "_buffers", None)
        if isinstance(buf, dict):
            buf.pop("total_ops", None)
            buf.pop("total_params", None)
        # 兜底：若被当作普通属性挂的，也删掉
...
class BaseModel(torch.nn.Module):
    ...
    def _profile_one_layer(self, m, x, dt):
        ...
        Args:
            ...
            ex (torch.Tensor): Th input data to the layer.
        ...
        if thop:
            _clear_thop_buffers(m)  # ★ 关键：每次 profile 前清理
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        # 这里是将enc_bboxes 添加到dec_bboxes的最前面 形成一个新的张量
        dec_bboxes = torch.cat([enc_bboxes.unsqueeze(0), dec_bboxes])  # (7, bs, 300, 5)
        ...
        return sum(loss.values()), torch.as_tensor(
            [loss[k].detach() for k in ["loss_giou", "loss_class", "loss_bbox", 'loss_angle']], device=img.device
        )
    ...
    def predict(self, x, profile=False, visualize=False, batch=None, augment=False, embed=None):
        ...
        for m in self.model[:-1]:  # except the head part
            ...
            # print(type(x), len(x) if isinstance(x, tuple) else x.shape)
            # print(m)
            if any(torch.isnan(t).any() for t in x if isinstance(t, torch.Tensor)):
                print(f"[Layer {m.i}] NaN detected after {m.type}: {m.f} -> {m.i}")
                for ti, t in enumerate(x if isinstance(x, (tuple, list)) else [x]):
                    if isinstance(t, torch.Tensor) and torch.isnan(t).any():
                        print(f"  tensor[{ti}] shape={t.shape}, nan={torch.isnan(t).sum().item()}/{t.numel()}, "
        ...
        if any(torch.isnan(t).any() for t in x if isinstance(t, torch.Tensor)):
            print("NaN detected in decoder embed")
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    base_modules = frozenset(
        {
            ...
            C2fCIB,
            A2C2f,
            SimpleStem, VisionClueMerge, VSSBlock_YOLO, XSSBlock,nn.Conv2d,SimAM,RepC3_BFAM,RepC3_WTConv
        }
    )
    # 需要重复的模块
    repeat_modules = frozenset(  # modules with 'repeat' arguments
        {
            ...
            C2PSA,
            A2C2f,
            nn.Conv2d, ChannelAttention_HSFPN, Multiply, Add,RepC3_BFAM,RepC3_WTConv
        }
    )
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            args = [ch[f], *args]
        elif m in frozenset({HGStem, HGBlock, HGBlock_EfficientViMBlock, HGBlock_AdaptiveRotatedConv2d,HGBlock_SimAM,HGBlock_DEConv}):
            c1, cm, c2 = ch[f], args[0], args[1]
            args = [c1, cm, c2, *args[2:]]
            if m in {HGBlock, HGBlock_EfficientViMBlock, HGBlock_AdaptiveRotatedConv2d,HGBlock_SimAM,HGBlock_DEConv}:
                args.insert(4, n)  # number of repeats
                n = 1
        elif m in {ResNetLayer, ResNetLayer_EfficientViMBlock}:
